...
if starting_pos_test:  # location of the tip of the projectile relative to the start of coil 1
    starting_pos_arr = [0.135, 0.175, 0.215, 0.25, 0.275, .315]
else:
    starting_pos_arr = [.25]
...
def main_loop(input):
    # Main testing loops #
    volt, num_turns, starting_pos, coil_2_threshold_dist, coil_3_threshold_dist, file_name = input[0], input[1], input[2], input[3], input[4], input[5]
    ...
    temp_path = "Models/" + file_name + ".fem"
            ...
            for starting_pos in starting_pos_arr:
                for coil_2_threshold_dist in coil_2_thresh_arr:
                    for coil_3_threshold_dist in coil_3_thresh_arr:
                        file_name = str(random.randint(1,100000000))
                        master_testing_arr.append([volt,num_turns,starting_pos,coil_2_threshold_dist,coil_3_threshold_dist, file_name])
    print(multiprocessing.cpu_count())
    pool = multiprocessing.Pool(2)
    pool.map(main_loop, master_testing_arr)
    pool.close()
